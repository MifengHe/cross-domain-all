(function () {
    ...
    var crossDomainEventBus = window.crossDomainEventBus = window.cdbus = new CrossDomainEventBus();
    ...
    var keys = "cross-domain-main.html";

    function CrossDomainSub(url) {
        if (typeof url != "string" || url.length == 0) {
            console.log(url.substr(-keys.length), keys);
        }

        var div = document.createElement("div");
        div.id = '_' + new Date().getTime() + Math.random();
        div.style.display = "none";
        div.innerHTML = _template._format(div.id, url);
    ...
    CrossDomainSub.prototype.dispatch = function(action, data, callback) {
    ...
    CrossDomainSub.prototype.action = function (action, onAction) {
        var _sub = this;
    ...
    CrossDomainSub.prototype.once = function (action, onAction) {
        var _sub = this;
        return crossDomainEventBus.once(action, function (payload) {
            ...
            callback(onAction(payload));
        });return crossDomainEventBus.once(action, onAction);
    };


    window.CrossDomainSub = CrossDomainSub;
})();